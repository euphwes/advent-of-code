...
def _parse_floor(floor):
    ...
    # Group the chips and generators, and strip the -g or -c tag so we're just left with elements.
...
def _score_state(e, floors):
    """ Score this state of the board, where lower scores mean the board is closer to being scored.
    This score is used for the "priority" of the state when being placed into the heap to check. """
...
def _calc_min_steps_to_solved(floors):
    ...
    while state_queue:
        ...
        for e, f in _get_available_steps(state.elevator, state.floors):
            ...
            new_state = PriorityStateGraphNode(_score_state(e, f), e, f, state.depth+1)
            heappush(state_queue, new_state)


@aoc_output_formatter(2016, 11, 1, 'minimum steps')
def part_one(floors):
    return _calc_min_steps_to_solved(floors)


@aoc_output_formatter(2016, 11, 2, 'minimum steps for real layout')
def part_two(floors):
    return _calc_min_steps_to_solved(floors)

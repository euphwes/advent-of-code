...
# Assign empty space character to something less visually intrusive, to make the ascii keypads
...
def _get_code(instructions, keypad, start_pos):
    ...
    for line in instructions:
        for step in line:
            # For each character in the instruction, move up, down, left, right accordingly,
            # to determine where your finger *might* end up
            if step == 'U':
                next_coords = (x, y-1)
            elif step == 'D':
                next_coords = (x, y+1)
            ...
            # the edge of the keypad), so just stay where you are. If it's a valid character, move
            # to that position.
            if next_char != ' ':
                x, y = next_coords
...
@aoc_output_formatter(2016, 2, 1, 'bathroom code')
...
@aoc_output_formatter(2016, 2, 2, 'correct bathroom code')

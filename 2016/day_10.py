...
def _evaluate_robots(instructions, stop_predicate):
    ...
    microchips it contains. After evaluating each instruction, check the stop predicate return the
    desired value when the stop predicate is satisfied. """
    ...
    while True:
        ...
        if 'gives' in step:
            ...
            # microchips yet, we can't evaluate it. Take this step and append it to the end
            # of the instructions, and we'll try to append it next time it comes around.
            donor_bot_microchips = robots[donor_bot]
            if not len(donor_bot_microchips) == 2:
                instructions.append(step)
                continue

            # Give the low recipient the donor robot's low-valued microchip, and the high recipient
            # the donor robot's high-valued microchip.
...
@aoc_output_formatter(2016, 10, 1, 'bot number which compared the target microchips')
def part_one(instructions):

    def stop_and_return(robots):
        """ Stop evaluating if we find the robot with microchips #17 and #61, and return the robot
        number which is holding those chips. """
...
@aoc_output_formatter(2016, 10, 2, 'product of the chip values in outputs 1-3')
def part_two(instructions):

    def stop_and_return(robots):
        """ Stop evaluating if we find that output 0, output 1, and output 2 all contain microchips,
        and return the product of the values of those microchips. """
...
def run(input_file):
    ...
    # Need to duplicate the instructions, because we've already chewed through the list evaluating
    # them in part one.
    instructions = get_input(input_file)

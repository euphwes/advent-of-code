...
PART_ONE_DESCRIPTION = "top crate on each stack"
PART_ONE_ANSWER = "SHMSDGZVC"

PART_TWO_DESCRIPTION = "CraneMover 9001 top crate on each stack"
PART_TWO_ANSWER = "VRZGHDFBQ"
...
def _parse_stacks(lines):
    ...
    while "1" not in (line := next(lines_iter)):
        ...
        # Consider 4 characters at a time from the line. Each chunk of 4 characters is
        # potentially a crate in the next stack. Start at stack #0.
        stack_ix = 0
        while line:
            ...
            # If there's a character remaining, that's a crate, present in the stack at
            # `stack_ix`
            if crate:
                stacks_dict[stack_ix].append(crate)

            # Increment which stack we might find a crate in during the next 4 chars we check.
            stack_ix += 1
...
def _parse_instructions(lines):
    """Parses the input instructions and returns a list of tuples of number of crates to be
    moved, and the (zero-indexed) source stack and target stack numbers."""

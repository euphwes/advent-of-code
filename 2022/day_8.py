...
def _is_visible(tree_coordinate, forest, forest_width, forest_height):
    ...
    visible_down  = not any(forest[(cx, y)] >= tree for y in range(cy + 1, forest_height))
    visible_left  = not any(forest[(x, cy)] >= tree for x in range(0, cx))
    visible_right = not any(forest[(x, cy)] >= tree for x in range(cx+1, forest_width))
...
def _scenic_score(tree_coordinate, forest, forest_width, forest_height):
    ...
    # of trees you find as you walk away from the target tree whose height is less than the target
    # tree. Include a point for the final (as tall or taller) tree you find.
...
@aoc_output_formatter(2022, 8, 1, 'number of trees visible outside the grid', assert_answer=1543)
def part_one(forest):

    width = max([x for x, y in forest.keys()]) + 1
    height = max([y for x, y in forest.keys()]) + 1

    return sum(1 for coord in forest.keys() if _is_visible(coord, forest, width, height))


@aoc_output_formatter(2022, 8, 2, 'maximum scenic score of a tree', assert_answer=595080)
def part_two(forest):

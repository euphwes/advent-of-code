...
def _build_next_row(prior_row):
    ...
    # For each tile in the new row, its state is determined by the tile at the same position in the
    # previous row. If that prior state is one of the states in the set above, the new tile is a
    # trap, otherwise it's a safe tile.
    next_row = ''
...
def _get_room_safe_tile_count(row, total_rows):
    """ Count the total number of safe tiles in the room, given a starting row and a total number of
    rows in the room. """
...
@aoc_output_formatter(2016, 18, 1, 'safe tile count, small room')
def part_one(first_row):
    return _get_room_safe_tile_count(first_row, 40)


@aoc_output_formatter(2016, 18, 2, 'safe tile count, big room')

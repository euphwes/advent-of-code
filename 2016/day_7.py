...
PART_ONE_DESCRIPTION = "number of IPs supporting TLS"
PART_ONE_ANSWER = 105

PART_TWO_DESCRIPTION = "number of IPs supporting SSL"
PART_TWO_ANSWER = 258
...
def _get_all_aba(val):
    """Returns all ABA sequences present in a string, which is a 3 character sequence of the
    form ABA where the inner letter B differs from the framing As."""
...
def _does_ip_support_tls(ip):
    """Returns whether the IPv7 address supports TLS. If the any hypernet sequences (within
    square brackets) contain an ABBA (character pair followed by its reverse), then address does
    not support TLS. If an ABBA is present anywhere else, it does support TLS."""

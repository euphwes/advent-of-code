...
def __normalize_blocklists(blocklist_ranges):
    ...
    for blocklist_range in blocklist_ranges:
        ...
        # If the new range starts at least 2 after the current ones ends, it's a brand new range.
        ...
        # If the new range starts exactly where the current one ends, or 1 immediately after, it's
        # a continuation of the range because these ranges are inclusive on both sides.
...
@aoc_output_formatter(2016, 20, 1, '')
def part_one(blocklist_ranges):
    # Normalize the blocklist ranges, and since we are confident the ranges are in order and don't
    # overlap, we can return the first number after the endpoint of the first blocklist range.
    return __normalize_blocklists(blocklist_ranges)[0][1] + 1


@aoc_output_formatter(2016, 20, 2, '')
def part_two(blocklist_ranges):
    count = 0
    # For each pair of ranges, figure out how many values fall after the first range but before the
    # second range, and add that to the running count.

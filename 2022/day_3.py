...
PART_ONE_DESCRIPTION = "combined priority of items shared between compartments"
PART_ONE_ANSWER = 7889

PART_TWO_DESCRIPTION = "combined priority of items shared between 3 grouped elves"
PART_TWO_ANSWER = 2825
...
def part_one(rucksacks):
    ...
    for sack in rucksacks:
        # Each rucksack has 2 compartments of equal size containing multiple items. Split the
        # sack in half to find each compartment.
        half = int(len(sack) / 2)
        compartment_1, compartment_2 = sack[:half], sack[half:]

        # Find all the items in each compartment and then find the items which are shared
        # between both.
...
def part_two(rucksacks):
    ...
    while rucksacks:
        ...
        # Then grab groups of 3 sacks and reduce the shared items to only include whatever is in
        # those sacks.
        for _ in range(3):

...
class IntcodeComputer:
    ...
    def __init__(self):
        ...
        self.opcode_map = {
            ...
            IntcodeComputer.OPCODE_ADJ_BASE: self.enact_adjust_relative_base,
        }
    ...
    def get_opcode_and_param_modes(self):
        ...
        raw_opcode = str(self.read_mem(self.instruction_ptr))
    ...
    def get_parameters_for_opcode(self, opcode):
        ...
        return [self.read_mem(i) for i in range(i+1, i+1+num_params)]
    ...
    def check_memory_size(self, idx):
        """ Checks that the memory is sufficiently sized to access a value at
        the provided index. If not, expand memory to fit. """

        if len(self.program) > idx:
            return

        difference = idx - len(self.program) + 1
        self.program.extend([0] * difference)


    def read_mem(self, idx):
        """ Reads from memory at the specified index. """

        self.check_memory_size(idx)
        return self.program[idx]


    def write_mem(self, idx, value):
        """ Writes the provided value to memory at the specified index. """
    ...
    def determine_param_value(self, param_id, param_mode):
        ...
        if param_mode == IntcodeComputer.PARAM_MODE_RELATIVE:
            return self.read_mem(self.relative_base + param_id)

        return self.read_mem(param_id)
    ...
    def enact_add(self, param1_with_mode, param2_with_mode, output_param):
        ...
        self.write_mem(output_idx, val1 + val2)
    ...
    def enact_input(self, target_param):
        ...
        self.write_mem(target_idx, input_value)
    ...
    def enact_less_than(self, param1_with_mode, param2_with_mode, output_param):
        ...
        self.write_mem(output_idx, 1 if val1 < val2 else 0)
    ...
    def enact_adjust_relative_base(self, param1_with_mode):
        """ Executes an ADJUST RELATIVE BASE instruction by incrementing or
        decrementing the current relative base by the value of the parameter. """

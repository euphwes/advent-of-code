...
def _v1_get_directory_size_map(terminal_output):
    ...
    File nodes and references to other Directory nodes, and size is calculated by the Directory. """
        ...
        if terminal_line.startswith('$ cd ..'):
            ...
            while terminal_output and not terminal_output[0].startswith('$'):
                ...
                if child.startswith('dir '):
                    ...
                    directory_path_node_map[full_dir_path].subdirectory_names.append(full_child_path)

                else:
                    # Otherwise the child is a file, add a File node to the current Directory node.
...
def _v2_get_directory_size_map(terminal_output):
    """ Second implementation, reads serially from the terminal output, maintaining a stack of the
    full paths of all directories we're in, and every time we read a file, add its size to all
    ...
    for line in terminal_output:
        ...
        # Don't need to directly do anything if we're reading an `ls` or a directory being listed
        # as `ls` output.
        ...
        # Navigating down a level? Add the full path to the stack.
            ...
            # Get the full path of this directory, which is just this directory one layer
            # deeper than the full path of the current path on the stack.
            if directory_stack:
...
@aoc_output_formatter(2022, 7, 1, 'total size of directories < 100K in size', assert_answer=1325919)
def part_one(terminal_output):
    ...
    candidates = [size for size in directory_path_size_map_v2.values() if size < 100_000]

    return sum(candidates)


@aoc_output_formatter(2022, 7, 2, 'size of smallest directory to delete', assert_answer=2050735)

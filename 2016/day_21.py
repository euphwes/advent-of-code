...
def _parse_input(lines):
    """ Parses the problem input and returns a list of instructions for scrambling the passcode. """
    ...
    for line in lines:
        if 'swap position' in line:
            ...
            parts = line.split(' ')
            instructions.append([InstructionCode.ROTATE_DIRECTION, parts[1], int(parts[2])])
...
def _rotate_based_on_letter(passcode, args, is_reverse=False):
    """ Rotate the passcode based on the index of the provided letter + 1. If the index was at least
    4, rotate 1 additional. """

    if is_reverse:
        for n in range(len(passcode)):
            start_candidate = _rotate(passcode, 'left', n)
            if _rotate_based_on_letter(start_candidate, args, is_reverse=False) == passcode:
...
@aoc_output_formatter(2016, 21, 1, '')
...
@aoc_output_formatter(2016, 21, 2, '')

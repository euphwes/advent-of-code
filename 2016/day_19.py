...
def _simulate_second_bogus_white_elephant(number_elves):
    """ Simulates the Cowboy Shootout problem. For my problem input, this takes a very long time to
    completed, around 24 hours. """
    ...
    while elf_count > 1:
        ...
        # Shift all the elves so the next elf remaining is at the 0th index.
        # This is why the simulation is so slow... but the logic is cleaner and easier to read than
        # maintaining a moving pointer to the gift recipient elf index.
        first = elves[0]
        elves = elves[1:]
        elves.append(first)
...
@aoc_output_formatter(2016, 19, 1, 'last elf standing, old rules')
def part_one(number_elves):
    ...
    # and got a hit at OEIS (Online Encyclopedia of Integer Sequences). Switched over to the closed
    # form of the solution which I sourced from there.

    # return _simulate_bogus_white_elephant(number_elves)
    return _A006257_josephus_closed_form(number_elves)


@aoc_output_formatter(2016, 19, 2, 'last elf standing, new rules')
